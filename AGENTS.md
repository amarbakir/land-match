...
- Consult `docs/superpowers/specs/` and `docs/superpowers/plans/` for existing designs before creating your own approach
...
**IMPORTANT**: Use **bd (beads)** for ALL issue tracking. No markdown TODOs or other trackers.

### Commands

```bash
bd ready --json                          # Check for unblocked work
bd create "title" -d "description" -t bug|feature|task -p 0-4 --json
bd create "title" -d "desc" -p 1 --deps discovered-from:<parent-id> --json  # Link discovered work
...
3. Work, test, commit
4. Discover new work? `bd create` with `--deps discovered-from:<parent-id>`
5. `bd close <id> --reason "Done"` — close after commit
