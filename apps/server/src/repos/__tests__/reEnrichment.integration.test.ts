...
function insertCandidate(address: string, status: 'pending' | 'partial' | 'failed' | 'enriched') {
  return listingRepo.insertListing({
    address,
    latitude: 36.6,
    longitude: -92.1,
    enrichmentStatus: status === 'pending' ? undefined : status,
  });
}
...
describe('re-enrichment repo queries (integration)', () => {
  it('re-enriching a listing replaces its enrichment row instead of violating the unique constraint', async () => {
    ...
    await listingRepo.insertEnrichment(listing.id, {
  ...
  it('selects only non-enriched rows with retry budget, oldest first', async () => {
    ...
    await pool.query('UPDATE listings SET enrichment_attempts = 5 WHERE id = $1', [capped.id]);
    ...
    await pool.query('UPDATE listings SET latitude = NULL, longitude = NULL WHERE id = $1', [noCoords.id]);

    const candidates = await listingRepo.findListingsNeedingEnrichment(10, 5);

    expect(candidates.map((c) => c.id)).toEqual([failed.id, partial.id]);
  });
  ...
  it('recordEnrichmentAttempt consumes retry budget and only updates status when given one', async () => {
    ...
    let { rows } = await pool.query('SELECT enrichment_status, enrichment_attempts FROM listings WHERE id = $1', [listing.id]);
    ...
    await listingRepo.recordEnrichmentAttempt(listing.id, 'enriched');
    ({ rows } = await pool.query('SELECT enrichment_status, enrichment_attempts FROM listings WHERE id = $1', [listing.id]));
