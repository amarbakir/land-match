...
export async function login(
  ...
  try {
    ...
    // Timing-equalized (see lib/password.ts): verification runs whether or not
    // the user exists, so response time doesn't reveal whether the email is
    // registered — and a truthy result is only possible against a real stored
    // hash.
    const valid = await verifyPassword(password, user?.passwordHash);
    if (!user || !valid) return err(ERR.INVALID_CREDENTIALS);
