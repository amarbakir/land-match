...
describe('userService', () => {
  describe('getNotificationPrefs', () => {
    it('returns default email channel when user has no prefs set (null column)', async () => {
      ...
      if (result.ok) {
        expect(result.data.alertChannel).toBe('email');
      }
    });
    ...
    it('returns stored prefs when user has valid preferences', async () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.data.alertChannel).toBe('sms');
      }
    });
  ...
  describe('updateNotificationPrefs', () => {
    it('updates and returns the new prefs', async () => {
      mockRepo.updateNotificationPrefs.mockResolvedValueOnce({
        ...USER_ROW,
        notificationPrefs: { alertChannel: 'push' },
        updatedAt: new Date('2026-02-01'),
      });

      const result = await userService.updateNotificationPrefs('user-1', { alertChannel: 'push' });

      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.data.alertChannel).toBe('push');
      }
    });
    ...
    it('passes the input to the repo for persistence', async () => {
      ...
      await userService.updateNotificationPrefs('user-1', { alertChannel: 'sms' });
    ...
    it('returns NOT_FOUND when user does not exist', async () => {
      ...
      const result = await userService.updateNotificationPrefs('nonexistent', { alertChannel: 'email' });
    ...
    it('returns INTERNAL_ERROR when repo throws', async () => {
      ...
      const result = await userService.updateNotificationPrefs('user-1', { alertChannel: 'email' });
